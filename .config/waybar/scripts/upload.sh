...
FILE="/tmp/.waybar_upload_$IFACE"

if [ ! -f $FILE ]; then
    echo $(cat /sys/class/net/$IFACE/statistics/tx_bytes) > $FILE
    echo " 0 KB/s"
    exit
fi

OLD=$(cat $FILE)
NEW=$(cat /sys/class/net/$IFACE/statistics/tx_bytes)
DIFF=$((NEW - OLD))
echo $NEW > $FILE
